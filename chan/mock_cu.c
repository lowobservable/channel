...
int mock_cu_arrange(struct mock_cu *mock_cu, uint8_t status, uint8_t limit)
{
    mock_cu->regs[REG_CONTROL] = (limit << 8) | (status << 1); // TODO

    return 0;
}

int mock_cu_assert(struct mock_cu *mock_cu, int8_t expected_command, int8_t expected_count)
{
    uint32_t status = mock_cu->regs[REG_STATUS];

    uint8_t command = (uint8_t) (status >> 8);
    uint8_t count = (uint8_t) (status >> 16);
